...
<!--
  ONE OF EVERY ELEMENT. This is the skill's style reference and its smoke test.

      python3 ../scripts/render.py --spec report_spec.yaml

  Read this file to learn the markup. Open 01_elements.docx to see the result.
  Both are committed, so neither can drift from the other without a rebuild
  catching it. If this renders and validates, the pipeline works.
-->

# EXECUTIVE SUMMARY

The executive summary opens the report and carries the findings. It uses Heading 1,
like every top-level section, and therefore starts its own page.

# 1.0 HEADINGS AND TEXT

Body text is Arial 10. Section numbers are typed into the heading, not generated —
Quarto's automatic numbering is switched off, because the front matter's table of
contents is a live Word field that reads the heading text.
...
A table needs no special markup. Write an ordinary pipe table and it picks up the
house look from the Word table style: a blue header row with white bold type, and
body rows banded white and pale blue.

The title is a **bold line above the table**, numbered by hand. Keep the numbering
sequential with `renumber_titles.py`.
...
Text after the table. Note that the prose never cites a table by its number —
numbers are reassigned when sections move, so references to them go stale
silently. Write "the table above" instead. The lint pass in `render.py` refuses
to build a report that breaks this rule.
...
*[An image would sit here, inserted as `![](path/to/figure.png){width=6.5in}`.
Figures belong in the project's own outputs folder and are produced by a script,
not drawn by hand.]*

# 4.0 MATHEMATICS

Inline mathematics works: the design effect is $1 + (m - 1)\rho$, where $\rho$ is
